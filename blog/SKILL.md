...
- **여백 규칙 (평소 스타일).** 빽빽하게 붙이지 말 것. 세션 제목(`##`) 위 5줄(구분선이 있으면 그 5줄은 구분선 '위'에, 구분선↔제목은 딱 붙임), 소제목(`###`) 위 3줄(단 세션 제목 바로 밑 첫 소제목은 1줄), 소제목↔바로 밑 사진은 딱 붙임, 사진↔설명글 딱 붙임, [사진+설명글] 뒤 4줄, 연속 본문 문단 사이 1줄. `paste_to_naver.py`가 토큰 기반으로 `<p><br></p>` 자동 삽입(`SPACE_BEFORE_SESSION=5 / SPACE_BEFORE_SUB=3 / SPACE_AFTER_TEXT=4`). 업로드 전 `preview.html`(로컬 http로 띄워 크롬 스크린샷)로 눈 확인 필수.
- **동영상은 클립보드로 안 붙는다.** 사진과 달리 네이버 자체 업로더를 거쳐야 한다.
  `_lib/upload_video.py <파일> [제목]`이 툴바 버튼부터 제목 입력·삽입까지 처리한다.
  대본에는 `[영상 자리 : 파일명.mp4]` 한 줄로 자리만 잡아 두고, 본문을 다 올린 뒤
  그 문단을 클릭해 캐럿을 옮기고 스크립트를 부르면 그 자리에 들어간다.
- **사진 워터마크**는 `_lib/watermark.py <입력폴더> <출력폴더> --scale 0.85`.
  op5321 기존 글에서 실측한 우하단 도현체 워터마크를 폭 비율로 재현한다.
