...
대본에서 한 줄에 사진 두 장(`![](a.jpg) ![](b.jpg)`)을 쓰면 네이버의 두 장 나란히 배치(imageStrip2)로 올라간다.
`--save`를 붙이면 끝에 상단 "저장"으로 임시저장한다. 인라인 서식이 없는 대본은 에디터 문서 데이터로
직접 쓰고(`_lib/se_doc.py`, 키보드·클립보드 미사용), 서식이 있으면 붙여넣기 경로로 자동 전환한다.
강제로 붙여넣기 경로를 쓰려면 `--paste`.
...
규칙 요약 (2026-09-23, 3-1~4-3 발행본에서 사용자가 직접 고친 방식을 기준으로 갱신):
- 사진마다 대사를 받아 적지 않습니다. 줄거리를 요약하고, 한 문장이 사진 두세 장에 걸쳐 이어지게 씁니다.
  긴 문장은 `\` 줄바꿈으로 두 줄로 끊습니다. 글은 편당 500~900자가 기준입니다.
- 대사만 바뀌는 연속 컷은 한 줄에 사진 두 장(`![](a.jpg) ![](b.jpg)`)으로 적어 두 장 묶음으로 올립니다.
  편당 4~9쌍, 곁가지 대화와 중복 컷은 15~25%를 뺍니다.
- 절 제목은 전부 `##`(구분선 + 노란 바탕)입니다. `###` 소제목은 쓰지 않습니다. 퀘스트 : 미션명,
  보스전 : 이름, vs 이름, 도입 컷신 절 이름 모두 `##`입니다.
- 화면에서 확인되는 게임플레이 정보(조작 가능 캐릭터, 약점 속성, 페이즈 전환)는 넣습니다.
  감상과 독자에게 던지는 질문은 사용자가 직접 쓰므로 `[감상 자리]` 줄만 2~3곳 남깁니다.
  확인하지 않은 전작 연결 문장은 쓰지 않습니다.
- 결과 화면, 챕터 메뉴, 배틀 챌린지 사진은 뺍니다.
- 영상은 편의 사진 촬영 시간대와 겹치는 촬영 건을 전부 확인해 스토리 영상은 빠짐없이 넣습니다.
  이어지는 한 장면이면 sessions를 합치고, 너무 길면 장면이 바뀌는 곳에서 in/out으로 나눕니다.
  앞부분 자르기는 컷신 앞에 조작 화면만 이어지는 경우에만 합니다. 영상 자리는 촬영 시각 순서대로 둡니다.
...
6. 업로드는 `blog/scripts/upload_to_editor.py "<편 폴더>/초안" --save`입니다. `meta.json`은 상대 경로라 어느 맥에서든 같습니다.
   제목·글·사진은 에디터 문서 데이터로 한 번에 쓰고(`_lib/se_doc.py`), 영상은 파일 선택 창 없이 올려
   `[영상 자리]` 문단으로 옮깁니다. 키보드·클립보드를 쓰지 않으므로 업로드 중 다른 창을 써도 됩니다.
   마지막 줄의 "미디어 순서 대본과 일치"와 임시저장 목록 개수로 결과를 확인합니다.
