...
3. **렌더링 후 가림이 들어간 사진을 전 장 Read로 열어 최종 검수한다.** 검출기가 원리상
   ...
   수동 원을 그린 사진은 `anonymize.save_with_exif(bgr, 원본경로, 출력경로)`로 저장한다.
   `cv2.imwrite`는 촬영 시각(EXIF)을 버려서, 나중에 카톡·스크린샷과 시간순으로 섞을 때
   그 사진만 자리를 잃는다(2026-09-26). `anonymize.py` 출력은 이제 EXIF를 유지한다.
