...
**내부 링크 치환 (2026-09-19 도입, 기본 동작)**: 본문에 걸린
`arnopark.tistory.com/<번호>` 링크는 이미 네이버로 옮겨 둔 글이 있으면 그
네이버 주소로 자동으로 바뀐다. 연재 글은 선행 글을 티스토리 주소로 링크해
두는데, 옮기고 나면 그 링크가 네이버 밖으로 나가고 티스토리 원본을 지운
뒤에는 아예 죽는다 (실측: `arnopark.tistory.com/566`은 403).

매핑은 `scripts/naver_backlinks.py`가 한다. 이관된 네이버 글에는
`해당 글은 티스토리 블로그 <URL>의 글을 마이그레이션한 글입니다.` 백링크가
박혀 있으므로, 네이버 블로그 내부 검색(`PostSearchList.naver`)으로 티스토리
URL을 그대로 찾으면 이관본이 나온다. 766편을 전수 크롤링할 필요가 없다.
검색이 토큰 분해로 566 대신 5660을 물어올 수 있어, 후보 글을 실제로 받아
`<host>/<번호>`가 정확히 있는지 확인한 뒤에만 채택한다. 결과는
`scripts/.backlink_cache.json`에 캐시된다.

바꾸는 대상은 세 곳이다. 링크 주소(`<a href>`), 링크 텍스트(티스토리는 링크
텍스트를 URL로 두는데 변환기가 href를 버리고 텍스트만 남기므로 이쪽이 실제
결과물이다), 그리고 링크 카드(`figure[data-ke-type=opengraph]`)의 주소와
`p.og-title`·`p.og-host` 텍스트다. 카드 제목은 **네이버 이관본의 제목**으로
갈아끼운다. 그대로 두면 네이버 글에 티스토리 원제(`[원신 - 팁] ...`)와
`arnopark.tistory.com`이 그대로 새어 들어간다.

이관본이 없는 링크는 건드리지 않고 원본 그대로 둔다. 푸터의 출처 표기
(`해당 글은 티스토리 블로그 ...`)도 그대로 둔다. 그건 출처라서 유지가 맞다.
끄려면 `--no-link-rewrite`.
