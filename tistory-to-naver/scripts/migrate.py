...
Usage:
  python3 migrate.py <TISTORY_URL> [--clear] [--no-style] [--blog-id op5321] [--title "..."]
...
def main():
    ...
    override_title = None
    if "--title" in flags:
        override_title = sys.argv[sys.argv.index("--title") + 1]
    override_tags = None
    if "--tags" in flags:
    ...
    if has_code:
        ...
        injector = os.path.join(
            ...
            "_lib", "inject_code_blocks.py")
        r = subprocess.run([sys.executable, injector])
        if r.returncode != 0:
